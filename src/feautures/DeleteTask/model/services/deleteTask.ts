...
export const deleteTask = createAsyncThunk<
  ...
  DeleteTaskProps,
  ThunkConfig<string>
>('toDo/task/deleteTask', async (taskForDelete, thunkAPI) => {
  const {extra, dispatch, rejectWithValue} = thunkAPI;
  const forDeleteData = {
    taskId: taskForDelete.taskId,
    todoId: taskForDelete.toDoId,
  };
  try {
    const response = await extra.api.delete<Task>('task/', {
      data: forDeleteData,
    });
    if (!response.data) {
